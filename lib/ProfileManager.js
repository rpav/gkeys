...
const {ValidKeyMap, ValidShiftedMap, ValidMouseMap, KeyFixMap} = require('./KeyData.js');
...
class ProfileManager {
    ...
    watchUpdate() {
        for(const [file, ev] of Object.entries(this._fsfiles)) {
            ...
            if(dir == 'profiles') {
                ...
                let prof = this.loadProfile(path.join(this.documentDir, file));

                // E.g. editing the current profile
                if(prof && cur.name == prof.name) this.delayedSetCurrentProfile(prof);
            } else if(path.basename(file) == 'settings.js') this.loadSettings();
        }
        this._fsfiles = {};
    }
    ...
    loadSettings() {
        ...
        prn("Loaded settings: ", settingsPath);
    }
    ...
    loadProfile(profilePath) {
        try {
            ...
            prn('Loaded profile: ', profilePath);

            return profile;
        } catch(e) {
            prn('Error loading profile: ', profilePath, ':');
            prn(e);
        }
    }
    ...
    setCurrentProfile(profile, conf = {}) {
        if(profile) {
            if(conf.autoSwitching) {
                this.autoSwitchHook.forEach(f => f(this._curProfile, profile));
            }
            this._curProfile = profile;
            this._curProfileData = this._curProfile._data;
            prn(conf.autoSwitching ? 'Auto-switch' : 'Switch', ' profile: \'', profile.name, '\'')
        }
    }
    ...
    _validateKey(k) {
        if(!k || typeof k == 'number' || ValidKeyMap[k] || ValidMouseMap[k]) return k;
        if(KeyFixMap[k]) return KeyFixMap[k];
        if(typeof k == 'function') return k;
        if(Array.isArray(k)) return k.map(k1 => this._validateKey(k1));
